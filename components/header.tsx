  ...
  return (
    <header className="z-[999] relative">
      <motion.div
        ...
        backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[41rem] 
        sm:rounded-full dark:bg-gray-950 dark:border-black/40 
        dark:bg-opacity-75 z-50 hidden md:flex"
      ...
      <ul className="hidden md:flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
           {links.map((link) => (
             <motion.li
               className="h-3/4 flex items-center justify-center relative z-50"
               key={link.hash}
             ...
             >
              <Link
                className={clsx(
                  "flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300 z-50",
                  {
